...
export default {
  ...
  methods: {
    getVueCode(name) {
      const source = vue[name]
      const highlight = Prism.highlight(source, Prism.languages.html, 'html')
      return highlight
    },
    getJSCode(name) {
      const source = js[name]
      const highlight = Prism.highlight(source, Prism.languages.javascript, 'javascript')
      return highlight
    }
  }
}
