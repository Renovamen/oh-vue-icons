...
function loadAliases(less) {
  const re = /.ri-([a-z0-9-]+)\s*:before { content: \s*"\\([0-9a-f]+)"; }/g
...
function loadFile(path) {
  return fs.readFileSync(
    require.resolve(`remixicon/${path}`),
    "utf8"
  )
}

function extractGlyphs(path, aliases, { dir, color, verbose }) {
  let glyphs = glyphs2json(loadFile(path), 'ri')

  mkdirpSync(dir)

  for (let g of glyphs) {
    for (let alias of aliases[g.unicode_hex] || []) {
      const path = `${dir}/ri-${alias}.svg`
...
export default function splitSVG(dir, color = "currentColor", verbose = false) {
  const aliases = loadAliases(loadFile("fonts/remixicon.less"))

  extractGlyphs("fonts/remixicon.svg", aliases, {
    dir,
    color,
    verbose
  })
